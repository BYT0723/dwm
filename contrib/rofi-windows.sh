...
#
# 接线示例（外部 scripts 仓 ~/.dwm/dwm-launcher.sh）：
#   windows) /path/to/dwm/contrib/rofi-windows.sh ;;
# 本仓 config.h 的 Mod+w 保持不动：LAUNCHCMD("windows")。
#
...
SEL=$(rofi -dmenu -i -p windows -format i -no-custom -show-icons <"$MENU_NUL" 2>/dev/null) || exit 0
