...
if [[ $Type == "video" ]]; then
    ...
    nohup xwinwrap -ov -fs -- mpv -wid WID "$1" --no-osc --no-osd-bar --loop-file --player-operation-mode=cplayer --no-input-default-bindings --input-conf=~/.dwm/configs/wallpaperKeyMap.conf --hwdec >/dev/null 2>&1 &
