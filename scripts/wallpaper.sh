...
echo "# This file is generated by dwm script, and editing is not recommended " >~/.dwm/background.sh
...
if [[ $Type == "video" ]]; then
    ...
    echo "xwinwrap -ov -fs -- mpv -wid WID "$1" --no-osc --no-osd-bar --loop-file --player-operation-mode=cplayer --no-input-default-bindings --input-conf=~/.dwm/configs/wallpaperKeyMap.conf" >>~/.dwm/background.sh
elif [[ $Type == "img" ]]; then
