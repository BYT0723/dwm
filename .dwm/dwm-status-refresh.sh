...
print_cpu(){
    ...
    isShowTemp=$(cat ~/.dwm/statusConf | grep "show_temp" | tail -n 1 | awk -F '=' '{print $2}')
...
print_disk(){
    ...
    echo -e "\x02﫭$disk_root "
}

print_date(){
    printf '\x01'
    isExp=$(cat ~/.dwm/statusConf | grep "date_exp" | tail -n 1 | awk -F '=' '{print $2}')
...
print_speed(){
    ...
    isExp=$(cat ~/.dwm/statusConf | grep "net_speed_exp" | tail -n 1 | awk -F '=' '{print $2}')
